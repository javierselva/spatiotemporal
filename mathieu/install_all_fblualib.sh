...
if [ $current -eq 1 ]; then
    ...
    #git clone -b patch-1 https://github.com/codeVerySlow/thpp/ /root/thpp --recursive
...
mv /app/build_thpp.sh $dir/thpp/thpp/build.sh
chmod +x $dir/thpp/thpp/build.sh
cd $dir/thpp/thpp
# Before execuiting build.sh, perform these changes: https://github.com/facebook/thpp/compare/master...codeVerySlow:patch-1
# These are for applying a patch to TH++ libraries
sed -i 's/return THTensor_(max)(values, indices, t, dim);/return THTensor_(max)(values, indices, t, dim, 1);/g' $dir/thpp/thpp/detail/TensorGeneric.h
sed -i 's/return THTensor_(min)(values, indices, t, dim);/return THTensor_(min)(values, indices, t, dim, 1);/g' $dir/thpp/thpp/detail/TensorGeneric.h
sed -i 's/return THTensor_(sum)(r, t, dim);/return THTensor_(sum)(r, t, dim, 1);/g' $dir/thpp/thpp/detail/TensorGeneric.h
sed -i 's/return THTensor_(prod)(r, t, dim);/return THTensor_(prod)(r, t, dim, 1);/g' $dir/thpp/thpp/detail/TensorGeneric.h
./build.sh
